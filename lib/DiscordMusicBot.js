...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    mongoose.connection.on("connected", () => {
      this.log("Database: MongoDB-01 | MongoDB is connected.", "ready");
    });
    mongoose.connection.on("err", (err) => {
      this.warn(`Database: MongoDB-01 | An error occured.`, "error");
    });
    mongoose.connection.on("disconnected", () => {
      this.warn("Database: Mongodb-01 | MongoDB is disconnected.");
    });
  ...
  LoadCommands() {
    ...
    let CommandsDir = path.join(__dirname, "..", "commands", "normal");
